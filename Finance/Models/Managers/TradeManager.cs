...
namespace Finance
{
    public partial class TradeManager
    {
        ...
        public TradeManager(Portfolio portfolio)
        {
            ...
            portfolio.PositionClosed += PositionClosedHandler;
        }
        ...
        public List<Trade> GetAllStoplosses(Security security)
        {
            var ret = TradeQueue.Where(trd => trd.TradeType == TradeType.Stop && trd.Security == security).ToList();
            return ret;
        }
        ...
        protected void ExecuteTrade(Trade trade, decimal executionPrice, DateTime AsOf)
        {
            ...
            executionPrice = TradingEnvironment.Instance.SlippageAdjustedPrice(executionPrice, trade.TradeActionBuySell);
        ...
        protected void PositionClosedHandler(object sender, PositionDataEventArgs e)
        {
            // Cancel all stoploss trades
            GetAllStoplosses(e.position.Security).ForEach(x => x.TradeStatus = TradeStatus.Cancelled);
        }
    }
}
