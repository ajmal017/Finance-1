...
{
    /// <summary>
    /// Implementation of an IEnvironment which reflects IBKR values and functions
    /// </summary>
    public partial class IbkrEnvironment : IEnvironment
    {
    ...
    public partial class IbkrEnvironment : IEnvironment
    {
        ...
        /// <param name="API">Flag indicating if the trade was placed through the API</param>
        /// <returns></returns>
        public decimal CommissionCharged(Trade trd, bool API)
        {
        ...
        /// <returns></returns>
        public decimal CommissionCharged(List<Trade> trds, bool API)
        {
            if (NegateCommissionForTesting)
                return 0.0m;
            ...
            try
            {
                return trds.Sum(x => CommissionCharged(x, API));
            }
            catch (Exception ex)
            {
                Console.WriteLine($"EXCEPTION:{GetCurrentMethod()}  {ex.Message}");
        ...
        {
            PriceBar currentBar = position.Security.GetPriceBar(AsOf);
            if (currentBar == null)
